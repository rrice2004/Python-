...
QUESTIONS = [
    ...
    ("bug_bounty", "Bug bounty program in place? (Y/N)"),          # NEW
    ("requires_install", "Does this require a software installation? (Y/N)"),  # NEW
    ("paywall", "Is the software behind a paywall? (Y/N)"),         # NEW
    ("security_patches", "Will the vendor provide security patches? (Y/N)"),  # NEW
...
def start_review():
    ...
    save_review(data, os.path.join(REVIEW_DIR, filename))
...
def continue_review():
    ...
    for key, question in QUESTIONS:
        ...
        prompt = f"{question} (current: {current}) " if current else question + " "
...
from reportlab.platypus import LongTable  # added import (PDF section only)
...
def generate_pdf():
    ...
    body_style = ParagraphStyle(
        ...
        fontSize=9,
        leading=12,
        wordWrap="CJK"  # critical for wrapping
    )
    ...
    def section(title, keys):
        ...
        for k in keys:
            ...
            rows.append([
                Paragraph(k.replace("_", " ").title(), body_style),
                Paragraph(ans or "&nbsp;", body_style)
            ])

        table = LongTable(
            rows,
            colWidths=[pdf.width * 0.3, pdf.width * 0.7],
            repeatRows=1
        )
    ...
    section("Security Practices",
        ["pentesting", "pentest_details", "bug_bounty",
         "requires_install", "paywall", "security_patches",
         "certifications", "auth_controls", "encryption"])
...
def generate_blank_word_report():
    ...
    for sec, keys in sections.items():
        ...
        for k in keys:
            ...
            row[0].text = qdict.get(k, k)
            row[1].text = ""

    doc.save("Blank_Software_Security_Review.docx")
    print("Blank Word report created: Blank_Software_Security_Review.docx")
